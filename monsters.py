...
class Monster: # All monsters use this class (In the future, boss-monsters will use a
    ...
    def __init__(self, name, hp, mp, attk, dfns, m_attk, m_dfns, spd, evad, lvl):
        self.name = name     # Name
        self.hp = hp         # Health
        self.mp = mp         # Mana
    ...
    def monst_name(self):
        ...
        if modifier == 'Foolish':
            self.m_attk -= 2
            self.m_dfns -= 2
        self.name = modifier + ' ' + self.name

def spawn_monster():
    global monster
    setup_vars()
    monster = Monster('', random.randint(5, 7), random.randint(2, 3), 2, 1, 2, 1, 2, 1, 1)
    monster.monst_level()
    monster.monst_name()