...
def battle_system():
    ...
    if monster.name[0] in vowels: # Remember to use proper grammar!
        ...
        a_an = 'A '
    static['dfns'] = player.dfns
    static['m_dfns'] = player.m_dfns
    print('-'*25)
...
def player_choice():
    while True:
        move = input('1: Attack; 2: Use Magic; 3: Defend; 4: Wait; 5: Run | Input #(1-5): ')
        if move.isdigit() and int(move) in range(1, 6): # Only return if "move" refers to a valid move
            return move
    ...
    while True:
        ...
        if move == '1': # Attack
            ...
            if magic.pick_cat(var, dodge):
                ...
                move = input('1: Attack; 2: Use Magic; 3: Defend; 4: Wait; 5: Run | Input #(1-5): ')
                if move.isdigit() and int(move) in range(1, 6): # Yes, it's redundant, but it works.
                    continue
        elif move == '3': # Defend (I'm planning on removing this later and
                          # replacing it with a spell that does the same thing)
            print("You focus your strength as you prepare for an oncoming attack! (+DEF, +M'DEF)")
            player.dfns += random.randint(1, 2)
            player.m_dfns += random.randint(1, 2)
            return
        elif move == '4': # Wait
...
def enemy_turn(var, dodge): # This is the Enemy's AI.
    ...
    elif monster.attk >= monster.m_attk and player.dfns: # Physical Attack
    ...
    elif monster.m_attk >= monster.attk and player.m_dfns < monster.m_attk and monster.mp >= 2: # Magic Attack
...
def after_battle(): # Assess the results of the battle
    global player
    global position
    player.dfns = static['dfns']
    player.m_dfns = static['dfns']
    while True:
        if monster.hp > 0 and player.hp <= 0:
            ...
            while True:
                ...
                try:
                    ...
                    continue
                if y_n in 'yes': # If you die, you return to O, O and are at half mana/health
                    position['x'] = 0
                    position['y'] = 0
            ...
            try:
                reward = int(math.sqrt(monster.lvl - player.lvl + 2) + player.ext_exp)*2
            except ValueError:
                reward = 1
            if reward <= 0:
                reward = 1
            player.exp += reward
            print("You've gained %s experience points!" % (reward))
            player.level_up()
            return
...
def bat_stats(): # Makes sure that the player and monster never have negative stats
                 # Display their stats after their fixed
    global player
    global monster
    if player.hp < 0:
        player.hp = 0
